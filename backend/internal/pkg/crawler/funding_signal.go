...
func (p *FundingSignalPlugin) searchBing(ctx context.Context, query string, limit int) []*biz.RawTopic {
	...
	if err != nil {
		p.log.Warnf("[FundingSignal] Bing HTML error for '%s': %v, fallback to RSS", query, err)
	...
	p.log.Warnf("[FundingSignal] Bing HTML got 0 for '%s', fallback to RSS", query)
	return p.searchBingRSS(ctx, query, limit)
}

func (p *FundingSignalPlugin) searchBingRSS(ctx context.Context, query string, limit int) []*biz.RawTopic {
	...
	if err != nil {
		p.log.Warnf("[FundingSignal] Bing RSS error for '%s': %v", query, err)
		return nil
	}
